...
var BloomPass = function (_Pass) {
  ...
  function BloomPass(strength, kernelSize, sigma, resolution) {
    ...
    _this.camera = new OrthographicCamera(-1, 1, 1, -1, 0, 1);
    _this.scene = new Scene();

    _this.quad = new Mesh(new PlaneBufferGeometry(2, 2), null);
...
var MattingPass = function (_Pass) {
  ...
  function MattingPass(primer, options) {
    ...
    _this.renderTargetX = new WebGLRenderTarget(width, height, pars);
    _this.renderTargetX.texture.name = 'MattingPass.x';
    _this.renderTargetY = new WebGLRenderTarget(width, height, pars);
    ...
    _this.mattingUniforms = UniformsUtils.clone(MattingShader.uniforms);
    ...
    _this.materialMatting = new ShaderMaterial({
    ...
    _this.edgeBlurUniforms = UniformsUtils.clone(EdgeBlurShader.uniforms);
    _this.edgeBlurUniforms.frameSize.value = new Vector2(width, height);

    _this.materialEdgeBlur = new ShaderMaterial({
...
var EffectPack = function () {
  function EffectPack(options) {
     ...
     */
    this.renderTarget = new WebGLRenderTarget(this.width, this.height, parameters$1);
...
var Viewer = function (_EventDispatcher) {
  ...
  function Viewer(options) {
     ...
     * @member {WebGLRenderer}
     */
    _this.renderer = new WebGLRenderer(new ViewConfig(options));
...
var Layer = function () {
  ...
  function Layer(options) {
     ...
     * @member {Mesh}
     */
    this.quad = new Mesh(new PlaneBufferGeometry(2, 2), new MeshBasicMaterial({
      transparent: true,
...
var XRLayer = function (_Layer) {
  ...
  function XRLayer(options) {
    ...
    _this.camera = new PerspectiveCamera(fov, aspect, near, far);
  ...
  createClass(CameraPrimer, [{
    ...
    value: function init(_ref2) {
      ...
      this.shader.texOrder.forEach(function (tex, idx) {
        ...
        _this2.uniforms[tex].value = new DataTexture(dataArray[idx], width * ss, height * ss, format);
      });
...
var TexturePrimer = function (_Primer) {
  ...
  function TexturePrimer(frame, options) {
    ...
    _this.texture = Utils.isString(frame) ? new TextureLoader().load(frame) : frame.tagName === 'VIDEO' ? new VideoTexture(frame) : frame.tagName === 'CANVAS' ? new CanvasTexture(frame) : frame.tagName === 'IMG' ? new Texture(frame) : null;
    ...
    _this.pigmentMat = new MeshBasicMaterial({
    ...
    _this.pigment = new Mesh(_this.pigmentGeo, _this.pigmentMat);
...
var AnchorRippling = function (_AnchorBase) {
  ...
  function AnchorRippling(options) {
    ...
    _this.map = Utils.isString(image) ? new TextureLoader().load(image) : null;

    _this.waves = new Group();
    _this.waves.position.z = -0.1;
    _this.uiFace = new Mesh(new PlaneBufferGeometry(width, height, segmentation, segmentation), new MeshBasicMaterial({ map: _this.map, transparent: true }));
  ...
  createClass(AnchorRippling, [{
    ...
    value: function initRippling(radius, segmentation, color, count, duration) {
      ...
      for (var i = 0; i <= count; i++) {
        var last = i === count;
        var circle = new Mesh(new CircleBufferGeometry(radius, segmentation), new MeshBasicMaterial({ color: new Color(color), transparent: true, depthTest: false }));
        if (!last) {
          var wait = waitTime * i;
          circle.runners({
...
var SphereWorld = function (_Object3D) {
  ...
  function SphereWorld(_ref) {
    ...
    _this.background = new Mesh(new SphereBufferGeometry(radius, segmentationX, segmentationY), new MeshBasicMaterial({ map: new TextureLoader().load(url), side: DoubleSide }));
...
var RATIO$1 = 0.15;
var RANGE$1 = 50;
var START$1 = new Vector2();
var END$1 = new Vector2();
...
var CylinderWorld = function (_Object3D) {
  ...
  function CylinderWorld(_ref) {
    ...
    _this.background = new Mesh(new CylinderBufferGeometry(radius, radius, height, radiusSegments, heightSegments, openEnded), new MeshBasicMaterial({ map: new TextureLoader().load(url), side: DoubleSide }));
  ...
  createClass(CylinderWorld, [{
    ...
    value: function _onMove(ev) {
      ...
      var DIFF = new Vector2().copy(END$1).sub(START$1).multiplyScalar(RATIO$1 * this.rotateSpeed);
    ...
    value: function updateDrag() {
      ...
      if (breaking < -180) compensate = -360;
      var speed = new Vector2(this.drag.x - this.gyro.lat * this.strength - this.dragX, target - (this.dragY + compensate));
          ...
          if (node) {
            (function () {
              ...
              for (var i = 0, il = targetNames.length; i < il; i++) {
                ...
                tracks.push(new TypedKeyframeTrack(targetNames[i] + '.' + PATH_PROPERTIES[target.path], AnimationUtils.arraySlice(inputAccessor.array, 0), AnimationUtils.arraySlice(outputAccessor.array, 0), interpolation));
              }
            })();
          }
        }
...
GLTFParser.prototype.loadNodes = function () {
  ...
  return _each(json.nodes, function (node) {
    ...
    var _node = node.isBone === true ? new Bone() : new Object3D();
...
export { Utils, Tween, Smooth, Orienter, BloomPass, ClearMaskPass, FilmPass, GlitchPass, MaskPass, MattingPass, RenderPass, ShaderPass, ConvolutionShader, CopyShader, FilmShader, FocusShader, ARGlue, Viewer, Layer, ARLayer, PrimerLayer, XRLayer, Primer, CameraPrimer, TexturePrimer, AnchorRippling, SphereWorld, CylinderWorld, GltfMagic };
